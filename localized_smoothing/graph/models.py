...
from torch_geometric.nn import GCNConv, GATConv, APPNP, GINConv, global_mean_pool, JumpingKnowledge, models
...
class SparseGCNConv(GCNConv):
    ...
    def forward(self, x, edge_idx, n, d):
        ...
        edge_idx, norm = gcn_norm(edge_idx, None, x.size(0), self.improved,
                                  self.add_self_loops, x.dtype)
...
class GCNLarge(nn.Module):
    ...
    def __init__(self, n_features, n_classes, n_hidden, p_dropout=0.5):
        ...
        self.conv6 = GCNConv(n_hidden, n_classes)
        self.p_dropout = p_dropout
    ...
    def forward(self, attr_idx, edge_idx, n, d):
        ...
        hidden = self.conv3(hidden, edge_idx)
        hidden = F.dropout(hidden, p=self.p_dropout, training=self.training)

        hidden = self.conv4(hidden, edge_idx)
        hidden = F.dropout(hidden, p=self.p_dropout, training=self.training)

        hidden = self.conv5(hidden, edge_idx)
        hidden = F.dropout(hidden, p=self.p_dropout, training=self.training)

        hidden = self.conv6(hidden, edge_idx)
...
class GAT(torch.nn.Module):
    ...
    def __init__(self, n_features, n_classes, n_hidden, k_heads, p_dropout=0.6):
        super().__init__()
        self.p_dropout = p_dropout
        self.conv1 = SparseGATConv(n_features,
                                   n_hidden,
                                   heads=k_heads,
                                   dropout=self.p_dropout)
        # On the Pubmed dataset, use heads=8 in conv2.
        self.conv2 = GATConv(n_hidden * k_heads,
                             n_classes,
                             heads=1,
                             concat=True,
                             dropout=self.p_dropout)
...
def get_model(name,
    ...
    ModelClass = name_map[name]
    model = ModelClass(n_features=d, n_classes=nc, **model_params)
    if pretrained_name != 'None' and pretrained_name is not None:
        if path is None:
            loaded = torch.load('./data/graphs/models/' + pretrained_name)
        else:
            loaded = torch.load(path + pretrained_name)
        try:
