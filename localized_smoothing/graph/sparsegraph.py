...
sparse_graph_properties = [
    ...
    'attr_names', 'edge_attr_names', 'class_names', 'metadata'
]
...
class SparseGraph:
    ...
    def __init__(self,
        ...
        if edge_attr_matrix is not None:
            if sp.isspmatrix(edge_attr_matrix):
                ...
                raise ValueError(
                    "Edge attribute matrix must be a sp.spmatrix or a np.ndarray (got {0} instead)."
    ...
    def calc_laplacian(self, normalized=False) -> sp.csr_matrix:
        degrees = np.asarray(self._adj_matrix.sum(axis=1)).reshape(-1)
        D = sp.diags(degrees)
        if not normalized:
            return D - self._adj_matrix
        #TODO: check why we have so many with zero degree and if there is a better way
        # to calculate the symmetric laplacian then
        I = sp.eye(self._adj_matrix.shape[0])
        Dinvhalf = sp.diags(1 / np.sqrt(degrees))
        Lsym = I - Dinvhalf @ self._adj_matrix @ Dinvhalf
        return Lsym
    ...
    def get_idx_to_edgeid_matrix(self) -> sp.csr_matrix:
        ...
        """
        return sp.csr_matrix((np.arange(self.adj_matrix.nnz),
                              self.adj_matrix.indices, self.adj_matrix.indptr),
    ...
    def to_undirected(self) -> 'SparseGraph':
        ...
        if len(dup_ridx) > 0:
            new_adj_matrix[dup_idx] = (new_adj_matrix[dup_idx] -
                                       self.adj_matrix[dup_idx]).A1
        ...
        if self.edge_attr_matrix is not None:
            ...
            if len(dup_ridx) > 0:
                ...
                if not np.allclose(self.edge_attr_matrix[dup_edgeids],
                                   self.edge_attr_matrix[dup_rev_edgeids]):
                    raise ValueError(
                        "Edge attributes of opposing edges differ.")
            ...
            if len(dup_ridx) > 0:
                new_edgeid_mat[dup_idx] = (new_edgeid_mat[dup_idx] -
                                           edgeid_mat[dup_idx]).A1
    ...
    def is_weighted(self) -> bool:
        ...
        """
        return np.any(
            np.unique(self.adj_matrix[self.adj_matrix.nonzero()].A1) != 1)
    ...
    def is_connected(self) -> bool:
        ...
        """
        return sp.csgraph.connected_components(self.adj_matrix,
    ...
    def __repr__(self) -> str:
        ...
        loop_string = 'has self-loops' if self.has_self_loops(
        ) else 'no self-loops'
        return (
            "<{}, {} and {} SparseGraph with {} edges ({}). Data: {}>".format(
                dir_string, weight_string, conn_string,
                self.num_edges(warn=False), loop_string, ', '.join(props)))
    ...
    def standardize(self,
                    ...
                    select_lcc: bool = True) -> 'SparseGraph':
    ...
    def unpack(
        self
    ) -> Tuple[sp.csr_matrix, Union[np.ndarray, sp.csr_matrix], Union[
            np.ndarray, sp.csr_matrix], Union[np.ndarray, sp.csr_matrix]]:
    ...
    def from_flat_dict(data_dict: Dict[str, Any]) -> 'SparseGraph':
        ...
        for key in data_dict.keys():
            if key.endswith('_data') or key.endswith('.data'):
                ...
                init_dict[matrix_name] = sp.csr_matrix(
                    (data_dict[mat_data], data_dict[mat_indices],
        ...
        unknown_keys = [
            key for key in init_dict.keys()
            if key not in sparse_graph_properties
        ]
        if len(unknown_keys) > 0:
            raise ValueError(
                "Input dictionary contains keys that are not SparseGraph properties ({})."
                .format(unknown_keys))
...
def flag_writeable(matrix: Union[np.ndarray, sp.csr_matrix], writeable: bool):
    if matrix is not None:
        if sp.isspmatrix(matrix):
            ...
            raise ValueError("Matrix must be an sp.spmatrix or an np.ndarray"
...
def create_subgraph(sparse_graph: 'gust.SparseGraph',
    ...
    if sparse_graph.edge_attr_matrix is None:
        ...
        keep_edge_idx = np.where(np.all(np.isin(old_idx, nodes_to_keep),
                                        axis=1))[0]
    ...
    return SparseGraph(adj_matrix, attr_matrix, edge_attr_matrix, labels,
                       node_names, sparse_graph.attr_names,
                       sparse_graph.edge_attr_names, sparse_graph.class_names,
                       sparse_graph.metadata)


def largest_connected_components(sparse_graph: 'gust.SparseGraph',
                                 n_components: int = 1) -> 'gust.SparseGraph':
    ...
    components_to_keep = np.argsort(
        component_sizes)[::-1][:
                               n_components]  # reverse order to sort descending
    nodes_to_keep = [
...
def remove_self_loops(sparse_graph: 'gust.SparseGraph') -> 'gust.SparseGraph':
    ...
    if num_self_loops > 0:
        ...
        return SparseGraph(adj_matrix, sparse_graph.attr_matrix,
                           edge_attr_matrix, sparse_graph.labels,
                           sparse_graph.node_names, sparse_graph.attr_names,
